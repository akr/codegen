...
\begin{gather*}
  \text{verify-refl:}~
    \dfrac{
      ...
      E[\Gamma] \vdash \mbox{eq\_refl}~t : t = t'
    } \breakrule
\end{gather*}
...
\begin{gather*}
  \text{verify-matchapp}:
    \dfrac{
      \rep{\underline{E[\Gamma]} \vdash p : \forall \mathit{cargs}, t\:\underline{\mathit{args}} = t'\:\underline{\mathit{args}'}}
    }{
      E[\Gamma] \vdash
        \begin{aligned}[t]
          ...
          & \quad
              \begin{aligned}
                & \kwmatch\:x'\:\kwwith\:\rep{C\:\mathit{cargs} \Rightarrow t}\:\kwend\:\mathit{args} = \\
                & \kwmatch\:x'\:\kwwith\:\rep{C\:\mathit{cargs} \Rightarrow t'}\:\kwend\:\mathit{args}'
              \end{aligned} \\
    ...
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        Since \codegen{} moves arguments to \kwmatch-expression into branches, $\mathit{args}'$ is empty in general.
      \end{minipage}
\end{gather*}
      ...
      \begin{aligned}[t]
        ...
        & q_j :=
          \begin{aligned}[t]
            ...
            & \kwend
          \end{aligned} \\
        & \mbox{$\mathit{cargs}$ is variables the constructors members}
      \end{aligned} \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
  ...
  \text{verify-letin-generic:}~
    \dfrac{
      \begin{aligned}
        ...
        & E[\Gamma; \ldefB{x}{t}; \ldefB{x'}{t'}; (H : x \simeq x')] \vdash q : u\:\mathit{args} = u'\:\mathit{args}'
      \end{aligned}
    }{
      E[\Gamma] \vdash
        \begin{aligned}[t]
...
\begin{gather*}
  \text{verify-letin-simple:}~
    \dfrac{
      \begin{aligned}
        & t \equiv t' \\
        & E[\Gamma; \ldefB{x}{t}] \vdash q : u\:\mathit{args} = \subst{u'}{x'}{x}\:\mathit{args}'
      \end{aligned}
    }{
      E[\Gamma] \vdash
        \begin{aligned}[t]
  ...
  \text{verify-apparg}:
    \dfrac{
      \begin{aligned}
        & E[\Gamma] \vdash x \not\equiv x' \\
        & E[\Gamma] \vdash p : x = x' \\
        & E[\Gamma] \vdash q : t\: \mathit{args}_0\: x\: \mathit{args} = t'\: \mathit{args}_0\: x\: \mathit{args}'
      \end{aligned}
    }{
      \kwrew\: p\: \kwin\: q : t\: \mathit{args}_0\: x\: \mathit{args} = t'\: \mathit{args}_0\: x'\: \mathit{args}'
    } \\
    \quad \mbox{where} \quad \kwrew\: p\: \kwin\: q := \mbox{eq\_ind}\: x\: (\lam{x}{t\: \mathit{args}_0\: x\: \mathit{args}})\: q\: x'\: p \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        When $x$ and $x'$ are functions, $p$ is an equality of functions.
        It needs functional extensionality (see the explanation for funext-fun).
      \end{minipage}
\end{gather*}
...
\begin{gather*}
  \text{funext-ind}:
    \dfrac{
      \begin{aligned}
        & \mbox{$t$ and $t'$ are not a function} \\
        & E[\Gamma] \vdash p : t \simeq t' \\
      \end{aligned}
    }{
      p : t = t'
...
\begin{gather*}
  \text{funext-fun}:
    \dfrac{
      ...
      p : \mbox{functional\_extensionality}\:t\:t'\:p : t = t'
    } \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
