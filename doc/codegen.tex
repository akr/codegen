...
\begin{gather*}
  \text{verify-fix}:
    \dfrac{
      \begin{aligned}
        & \{\rep{f}\} \supseteq \{\rep{g}\} \quad f_\textrm{s} \in  \{\rep{g}\} \\
        & \forall h \in \{\rep{g}\}, (\FV(t_h) \cup \FV(u_h)) \cap (\{\rep{f}\} \backslash \{\rep{g}\}) = \varnothing \quad \mbox{($\{\rep{f}\} \backslash \{\rep{g}\}$ are unreachable from $f_\textrm{s}$)} \\
        % & h_1 = |\rep{f}| \\
        % & h_2 = |\rep{g}| \\
        & \forall h \in \{\rep{g}\}, E[\Gamma]; \rep{\mathit{IH} : F \simeq G} \vdash p_{h} : \substm{t_{h}}{F}{f} \simeq \substm{u_{h}}{G}{g}
      \end{aligned}
    }{
      \begin{aligned}
        & (\kwfix\:\rep{\mathit{IH} / k : (F \simeq G) := \mathrm{IH}}\:\kwfor\:\mathit{IH}_{f_\textrm{s}})\:b_1 \ldots b_{\NA_{f_\textrm{s}}}
          : F_{f_\textrm{s}}\:b_1 \ldots b_{\NA_{f_\textrm{s}}} = G_{f_\textrm{s}}\:b_1 \ldots b_{\NA_{f_\textrm{s}}}
      \end{aligned}
    } \\
    \quad \mbox{where} \quad
      \begin{aligned}[t]
        ...
        & \mbox{$T_h$ is the type of $h$} \\
        & \mbox{$t_h$ is the function term for $h$ in the LHS fixpoint} \\
        & \mbox{$u_h$ is the function term for $h$ in the RHS fixpoint} \\
        & \mbox{$f_\textrm{s}$ specifies the start function of the LHS and RHS fixpoint} \\
        & \mbox{$\mathit{params}_h$ is the inductive type parameters of the decreasing argument of $h$} \\
        & F_{h} := \kwfix\:\rep{f/k_f:T_f:=t_f}\:\kwfor\:{h} \\
        & G_{h} := \kwfix\:\rep{g/k_g:T_g:=u_g}\:\kwfor\:{h} \\
        & \mathrm{IH}_{h} :=
          \begin{aligned}[t]
            & \lambda a_1 \ldots a_{\NA_{h}} . \\
            & \kwlet\:H := p_{h}\:\kwin \\
            & \kwmatch\:a_{k_{h}}\:\kwas\:x\:\kwreturn \\
            & \quad F_{h}\:a_1 \ldots a_{k_{h}-1}\:x\:a_{k_{h}+1} \ldots a_{\NA_{h}} = G_{h}\:a_1 \ldots a_{k_{h}-1}\:x\:a_{k_{h}+1} \ldots a_{\NA_{h}} \\
            & \kwwith \\
            & \rep{|\:C\:\mathit{cargs} \Rightarrow \underline{H\:a_1 \ldots a_{k_{h}-1}}\:(C\:\mathit{params}\:\mathit{cargs})\:\underline{a_{k_{h}+1} \ldots a_{\NA_{h}}}} \\
            & \kwend
          \end{aligned}
      \end{aligned} \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        This verify-fix rule builds a proof term for extensional equality of fixpoints ($\kwfix\ldots\kwfor\:\mathit{IH}_{f_\textrm{s}}$) from extensional equality of iota-fix reduced forms ($p_h$).
