...
\begin{align*}
  K =&\ [] \\
    ...
    |&\ \letin{x}{K:T}{t} \\
    |&\ \letin{x}{t:T}{K} \\
    |&\ \omatch{K}{\repop{C\:\rep{x{:}T}\Rightarrow u}{|}} \\
    |&\ \omatch{t}{\repopi{(C_l\:\rep{x_l{:}T_l}\Rightarrow u_l)}{|}{1\leq l < i} | (C_i\:\rep{x_i{:}T_i}\Rightarrow K) | \repopi{(C_l\:\rep{x_l{:}T_l}\Rightarrow u_l)}{|}{i < l \leq h}  } \\
    |&\ \ofix{\repopi{(f_l/k_l{:}T_l:=t_l)}{\kwwith}{1\leq l < i} (f_i/k_i{:}T_i:=K) \repopi{(f_l/k_l{:}T_l:=t_l)}{\kwwith}{i < l \leq h}}{f_j}
\end{align*}

\subsection{Free Variables}

$\FV(t)$ means the free variables of $t$.
...
\begin{align*}
  ...
  \FV(\ofix{\rep{f/k{:}T:=t}}{f_j}) &= (\tbigcup_i \FV(t_i)) - \{\rep{f}\}
\end{align*}
{\small Note:
\begin{itemize}
...
\subsection{Bound Variables of Syntactic Context}

$\BV(K)$ means the bound variables usable in the hole of the context $K$.

\begin{align*}
  \BV([]) &= \varnothing \\
  \BV(\lamT{x}{T}{K}) &= \{x\} \cup \BV(K) \\
  \BV(K\:t) &= \BV(K) \\
  \BV(t\:K) &= \BV(K) \\
  \BV(\letin{x}{K:T}{t}) &= BV(K) \\
  \BV(\letin{x}{t:T}{K}) &= \{x\} \cup \BV(K) \\
  \BV(\omatch{K}{\rep{C\:\rep{x{:}T}\Rightarrow u}}) &= \BV(K) \\
  \BV(\omatch{t}{\repi{(C_l\:\rep{x_l{:}T_l}\Rightarrow u_l)}{1\leq l < i} (C_i\:\rep{x_i{:}T_i}\Rightarrow K) \repi{(C_l\:\rep{x_l{:}T_l}\Rightarrow u_l)}{i < l \leq h}} &= \{\rep{x_i}\} \cup \BV(K) \\
  \BV(\ofix{\repi{(f_l/k_l{:}T_l:=t_l)}{1\leq l < i} (f_i/k_i{:}T_i:=K) \repi{(f_l/k_l{:}T_l:=t_l)}{i < l \leq h}}{f_j}) &= \{\rep{f}\} \cup \BV(K)
\end{align*}
