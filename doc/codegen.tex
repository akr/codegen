...
\lstdefinelanguage{coq}{
  ...
  morekeywords=[4]{CodeGen,GlobalInline,LocalInline,StaticArgs},
...
\newcommand{\TRLF}[1]{\TRop_\text{LF}\BRA{#1}}
\newcommand{\TREXP}[1]{\TRop_\text{EXP}\BRA{#1}}
\newcommand{\tr}{\mathit{TR}}

\DeclareMathOperator{\INLop}{INL}
\newcommand{\INL}[1]{\INLop_\text{LF}\BRA{#1}}

\DeclareMathOperator{\RNTop}{RNT}
\newcommand{\RNTLF}[1]{\RNTop_\text{LF}\BRA{#1}}
\newcommand{\RNTEXP}[1]{\RNTop_\text{EXP}\BRA{#1}}

\DeclareMathOperator{\Nop}{N}
\DeclareMathOperator{\Top}{T}

\newcommand{\NLF}[1]{\Nop_\text{LF}\BRA{#1}}
\newcommand{\NEXP}[1]{\Nop_\text{EXP}\BRA{#1}}
\newcommand{\TLF}[1]{\Top_\text{LF}\BRA{#1}}
\newcommand{\TEXP}[1]{\Top_\text{EXP}\BRA{#1}}
...
$\RNTLF{t} = (\INL{t}, \TRLF{t}, \NLF{t}, \TLF{t})$ classify variables in $t$.
\begin{align*}
  \INL{t} &: \text{$t$ is inlinable or not ($\true$ or $\false$)} \\
  \TRLF{t} &: \text{tail-recursive fixpoint bounded functions that do not need to be real functions in $t$} \\
  \NLF{t} &: \text{free variables at non-tail positions of $t$} \\
  \TLF{t} &: \text{free variables at tail positions of $t$}
\end{align*}

``tail position'' is extended to the function position of the application at a tail position. \\
$\TRop_\text{LF}$ distinguishes fixpoint bounded functions translatable without actual functions (but with \kwgoto{}) or not.

We also define $\RNTEXP{t} = (\TREXP{t}, \NEXP{t}, \TEXP{t})$.
$\RNTLF{t}$ and $\RNTEXP{t}$ are mutually recursive.
$\RNTLF{t}$ traverses abstractions and fixpoints ($\tD$ and $\tD'$).
$\RNTEXP{t}$ traverses other constructs ($\tL$, $\tM$, $\tE$, and $\tF$).

\begin{align*}
  \RNTLF{\lam{x}{t}} &= (\INL{t}, \TRLF{t}, \NLF{t}-\{x\}, \TLF{t} - \{x\}) \\
  \RNTLF{\ofix{\rep{f:=t}}{f_j}} &=
    \begin{cases}
      \begin{aligned}[t]
        \bigl(& \true, \\
              & \tbigcup_i \TRLF{t_i} \cup \{\rep{f}\}, \\
              & \tbigcup_i \NLF{t_i} - \{\rep{f}\}, \\
              & \tbigcup_i \TLF{t_i} - \{\rep{f}\} \bigr)
      \end{aligned}
        & \begin{aligned}[t]
          & \text{the fixpoint is not higher order} \wedge {} \\
          & \bigl(\tbigcup_i \NLF{t_i} \cap \{\rep{f}\} = \varnothing\bigr) \wedge {} \\
          & \forall_i ~\INL{t_i}
          \end{aligned} \\
      \begin{aligned}[t]
        \bigl(& \false, \\
              & \tbigcup_i \TRLF{t_i}, \\
              & \tbigcup_i (\NLF{t_i} \cup \TLF{t_i}) - \{\rep{f}\}, \\
              & \varnothing \bigr)
      \end{aligned}
        & \text{otherwise}
    \end{cases} \\
  \RNTLF{t} &= (\true, \TREXP{t}, \NEXP{t}, \TEXP{t}) \\
    & \text{where}~\text{$t$ is not abstraction nor fixpoint}
\end{align*}
\begin{align*}
  \RNTEXP{x\:\rep{y}} &= (\varnothing, \{\rep{y}\}, \{x\}) \\
  \RNTEXP{f\:\rep{y}} &= (\varnothing, \{\rep{y}\}, \{f\}) \\
  \RNTEXP{c\:\rep{y}} &= (\varnothing, \{\rep{y}\}, \varnothing) \\
  \RNTEXP{C\:\rep{y}} &= (\varnothing, \{\rep{y}\}, \varnothing) \\
  \RNTEXP{\letin{x}{t}{u}} &= (\TREXP{t} \cup \TREXP{u}, \NEXP{t} \cup \TEXP{t} \cup \NEXP{u} - \{x\}, \TEXP{u} - \{x\}) \\
  \RNTEXP{\omatch{y}{\rep{C\:\rep{x}\Rightarrow t}}} &= \bigl(\tbigcup_i \TREXP{t_i}, \tbigcup_i \NEXP{t_i} - \{\rep{x_i}\}, \tbigcup_i \TEXP{t_i} - \{\rep{x_i}\} \bigr) \\
  \RNTEXP{t} &= (\TRLF{t}, \NEXP{t} \cup \TEXP{t}, \varnothing) \\
    & \text{where}~t = \lam{x}{u} \\
  \RNTEXP{t\:\rep{y}} &=
    \begin{cases}
      (\TRLF{t}, \NEXP{t} \cup \TEXP{t}, \varnothing) & |y| = 0 \\
      (\TRLF{t}, \NEXP{t} \cup \{\rep{y}\}, \TEXP{t}) & \text{otherwise}
    \end{cases} \\
    & \text{where}~t = \ofix{\rep{f:=u}}{f_j}
\end{align*}
...
\begin{itemize}
  ...
  \item $\tr = \TRLF{t}$ where the translating function is defined as \kwDefinition~$c := t$.
