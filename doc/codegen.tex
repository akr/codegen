...
\subsection{Eta Expansion for Function Bodies}\label{sec:eta-expand-bodies}
We apply eta-expansion to function bodies of top-level functions and fix-bounded functions.
This makes beta-var applicable to the bodies.
...
\[
  ...
  \begin{cases}
    ...
    (\lam{x}{\E{u}{}{p+q-1}})\:x_1\ldots x_p                                                             & (t = \lam{x}{u}) \wedge ((p = 0) \wedge (r = 0)) \\
    \F{t\:x_1\ldots x_p}                                                                                              & (t = \lam{x}{u}) \wedge (r > 0) \\
...
\begin{gather*}
  \text{borrow-abs-fun:}~
    \dfrac{
      \begin{gathered}
        ...
        B' = (B - \{x_1,\ldots x_n\}) ~\text{filtered with the type of $T$} \\
      \end{gathered}
    }{
  ...
  \text{borrow-fix-fun:}~
    \dfrac{
      \begin{gathered}
        \text{$t$ is a fixpoint} \\
        E[\Gamma'::\lassum{f_1^\varnothing}{T_1}::\cdots::\lassum{f_n^\varnothing}{T_n}] \vdash t_i ~|~ B_i \\
...
{\small Note:
\begin{itemize}
  \item $\forall x, (\Delta_1 \oplus \Delta_2)(x) = \Delta_1(x) + \Delta_2(x)$ \text{where $\Delta(x) = 0$ if $\Delta$ doesn't contain a map from $x$}
\end{itemize}}
