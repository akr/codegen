...
{\small Note:
\begin{itemize}
  \item $u, a, b, c$ represents a term as $t$. \\ $y, z,$ and $f$ represent a variable as $x$. \\ $U$ represents a type as $T$.
...
\begin{gather*}
  \text{eta-top:}~
    \dfrac{
      ...
      \text{$t$ is a entire function}
    }{
      E[] \vdash t \reltri \lamT{x}{T}{(t\:x)}
    } \\
  \text{eta-fix:}~
    \dfrac{
      E[\Gamma;\rep{\lassum{f}{T}}] \vdash t : \prodT{x}{U_1}{U_2} \quad
      \text{$t$ is not an abstraction nor fixpoint}
    }{
      E[\Gamma] \vdash \ofix{\overline{f/k{:}T:=t}}{f_j} \reltri \ofix{\overline{f/k{:}T:=(\lamT{x}{U_1}{(t\:x)})}}{f_j}
    } \\
  \text{eta-abs:}~
    \dfrac{
      E[\Gamma;\lassum{y}{T}] \vdash t : \prodT{x}{U_1}{U_2} \quad
      \text{$t$ is not an abstraction nor fixpoint}
    }{
      E[\Gamma] \vdash \lamT{y}{T}{t} \reltri \lamT{y}{T}{(\lamT{x}{U_1}{(t\:x)})}
    }
\end{gather*}

This transformations makes a term in following syntax.
The entire function is represented as $t_F$.

\begin{align*}
  t_F =&\ \lamT{x}{T}{t_F} \\
      |&\ \ofix{\overline{f/k{:}T:=t_F}}{f_j} \\
      |&\ t   & \text{the type of $t$ is an inductive type} \\
  t =&\ x \\
    |&\ c \\
    |&\ C \\
    |&\ T \\
    ...
    |&\ t_F  & \text{$t_F$ is abstraction or fixpoint} \\
\end{align*}
    ...
    \dfrac
    {
      ...
      \end{gathered}
        ...
        E[\Gamma] \vdash\;
          ...
          \reltri
          \overline{\letinB{f'}{\ofix{\underline{(\overline{f/k:=t})}}{f}}} \: \substm{t_j}{f}{f'} \: \overline{x}
    } \\
  \text{iota-fix-var':}~
    \dfrac
    {
...
S-reductions transforms application to restrict constructions at function positions of applications.
\begin{itemize}
  ...
  \item zeta-flat removes a let-in at the first term of a let-in.
\end{itemize}
Also, types cannot be a function.
We treat multi-arguments application as single application, application is not occur at the function position of an application.
Thus, function position can be variable, constant, construcor, or fixpoint in the S-normal form.

\begin{align*}
  t^\textrm{D} =&\ \lamT{x}{T}{t^\textrm{D}} \\
      |&\ \ofix{\overline{f/k{:}T:=t^\textrm{D}}}{f_j} \\
      |&\ t^\textrm{L}   & \text{the type of $t^\textrm{L}$ is an inductive type} \\
  t^\textrm{L} =&\ \letinM{x}{t^\textrm{M}:T}{t^\textrm{M}} \\
  t^\textrm{M} =&\ \omatch{x}{\overline{C\:\overline{x}\Rightarrow t^\textrm{L}}} \\
      |&\ t^\textrm{E} \\
  t^\textrm{E} =&\ x ~|~ c ~|~ C ~|~ T \\
      |&\ t^\textrm{F}\:\rep{x} & \text{$0 < |x|$} \\
      |&\ t^\textrm{D}  & \text{$t^\textrm{D}$ is abstraction or fixpoint} \\
  t^\textrm{F} =&\ x ~|~ c ~|~ C ~|~ \ofix{\overline{f/k{:}T:=t^\textrm{D}}}{f_j}
\end{align*}
