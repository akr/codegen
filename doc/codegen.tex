...
\renewcommand{\_}{\underline{\phantom{t}}}
...
\newcommand{\omatchasinret}[5]{\kwmatch\:#1\:\kwas\:#2\:\kwin\:#3\:\kwreturn\:#4\:\kwwith\:{#5}\:\kwend}
...
\DeclareMathOperator{\BZop}{BZ}
\newcommand{\BZ}[1]{\BZop\BRA{#1}}
...
\begin{align*}
  t &= x & \text{v-variable} \\
    ...
    &\bnfor \omatchasinret{t}{y}{V\:\rep{z}}{U}{\repop{C\:\rep{x{:}T}\Rightarrow u}{|}} & \text{conditional} \\
...
\begin{itemize}
  \item $u, a, b$ represents a term as $t$. \\
    ...
    $U, V, W$ represents a type and type-constructor as $T$.
  ...
  \item We omit the elimination predicate (\kwas-\kwin-\kwreturn{} clause of \kwmatch-expression) when it is not interesting.
  \item We omitted the dummy parameters (underscores between $C$ and $\rep{x{:}T}$ and betwenn $V$ and $\rep{z}$) in conditionals.
...
\begin{gather*}
  ...
  \text{iota-fix:}~
    \dfrac{
      ...
      E[\Gamma] \vdash\; (\ofix{\rep{f/k:=t}}{f_j})\:\rep{u} \reltri\; \substm{t_j}{f}{\ofix{\underline{(\rep{f/k:=t})}}{f_j}} \: \rep{u}
    } \\
  \text{eta expansion:}~
...
Note that we don't apply eta-expansion for partial application in let-in binding.
The computation in the function-position and arguments of the partial application should be evaluated before closure generation.
...
The ``Unreachable Fixfunc Deletion'' drops unreachable functions in mutually-recursive fixpoints as follows.
...
\begin{gather*}
  \text{verify-applyhyp:}~
    \dfrac{
      (\mathit{H} : t \simeq t') \in \Gamma
    }{
      E[\Gamma] \vdash \mathit{H}\:\mathit{args} : t\ \mathit{args} = t'\ \mathit{args}
    } \breakrule
\end{gather*}
...
\begin{gather*}
  \text{verify-matchapp}:
    \dfrac{
      \rep{\underline{E[\Gamma]} \vdash p : \forall \mathit{cargs},\: \BZ{t\:\underline{\mathit{margs}}} = \BZ{t'\:\underline{\mathit{margs}'}}}
    }{
      E[\Gamma] \vdash
        \begin{aligned}[t]
          ...
          & \quad
              \begin{aligned}
                {} & \mathrel{\phantom{=}} \kwmatch\:x'\:\kwwith\:\rep{C\:\mathit{cargs} \Rightarrow t}\:\kwend\:\mathit{margs} \\
    ...
    \BZ{t\:y\:\rep{z}} =
      \begin{cases}
        \BZ{\subst{u}{x}{y}\:\rep{z}} & \text{if $t$ is $\lam{x}{u}$} \\
        \letin{x}{u}{\BZ{u'\:y\:\rep{z}}} & \text{if $t$ is $\letin{x}{u}{u'}$} \\
        t\:y\:\rep{z} & \text{otherwise}
      \end{cases} \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        \begin{itemize}
          \item Since \codegen{} moves arguments for a \kwmatch-expression into the branches, $t' = t\ \mathit{margs}$ and $\mathit{margs}'$ is empty in general.
          \item $\BZop$ moves the arguments into let-in body (zeta-app) and apply beta-var reduction as much as possible.  This transforms removes redundant applications.
        \end{itemize}
      \end{minipage}
\end{gather*}
  ...
  \text{verify-fix}:
    \dfrac{
      \begin{aligned}
        ...
        & \forall i \in \{1\ldots h'\},\: T_{\sigma(i)} \equiv T'_i \qquad \mbox{(function type equality)} \\
    ...
    \quad \mbox{where} \quad
      \begin{aligned}[t]
        & F_{i} := \kwfix\:\repi{f/k\!:\!T:=t}{j=1\ldots h}\:\kwfor\:{f_i} \\
        & F'_{i} := \kwfix\:\repi{f'/k'\!:\!T':=t'}{j=1\ldots h'}\:\kwfor\:{f'_i} \\
        ...
        & q_j :=
          \begin{aligned}[t]
           & \lambda x_1 \ldots x_{n_j} . \\
            & \kwlet\:H := p_{j}\:\kwin \\
            & \kwmatch\:x_{k'_j}\:\kwas\:y\:\kwreturn \\
            & \quad F_{\sigma(j)}\:x_1 \ldots x_{k'_j-1}\:y\:x_{k'_j+1} \ldots x_{n_j} = F'_{j}\:x_1 \ldots x_{k'_j-1}\:y\:x_{k'_j+1} \ldots x_{n_j} \\
            & \kwwith \\
            & \rep{\:C\:\mathit{cargs} \Rightarrow \underline{H\:x_1 \ldots x_{k'_j-1}}\:(C\:\underline{\mathit{params}_j}\:\mathit{cargs})\:\underline{x_{k'_j+1} \ldots x_{n_j}}} \\
            & \kwend
          \end{aligned} \\
        & n_j := \NA_{F'_j} \qquad \mbox{(number of arguments of $F'_j$)} \\
    ...
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        ...
        The unreachable functions are $\{\rep{f}\} \backslash \{\rep{f'}\}$ because our transformation does not change the function names in the fixpoints.
        ...
        The let-in expression to bind $H$ prevents copy of the proof term $p_j$ for each constructor.
      \end{minipage}
    \breakrule
\end{gather*}
...
\begin{gather*}
  \text{verify-apparg}:
    ...
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        When $x$ and $x'$ are function arguments for higher order functions $t$ and $t'$, $p$ is an equality of functions.
...
The verify-letin-generic rule introduces $x \simeq x'$ which is not usable for $\mathsf{eq\_ind}$ which require $x = x'$.
$\mathsf{functional\_extensionality}$ (and ``verify-forall'' rule) is used to convert the former to the later.
They are used multiple times if $x$ and $x'$ are functions with multiple arguments.

The reason \codegen{} uses $\mathsf{functional\_extensionality}$ is to automate the verification process.
...
The equality (\lstinline!Coq.Init.Logic.eq!, $t = t'$) and extensional equality ($\forall \mathit{args}, t\:\mathit{args}=t'\:\mathit{args}$) are not suitable for such type.
