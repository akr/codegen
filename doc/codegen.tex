...
\DeclareMathOperator{\OVARS}{OVARS}
...
We define outer-variables for recursion.

\codegen{} translates a recursive call to C function invocation (if \kwgoto{} is not usable).
For example, the fixpoint in
$\letin{\texttt{y}}{\ldots}{\ofix{\texttt{f}:=\lam{\texttt{x}}{\ldots \texttt{f}\:\texttt{x'} \ldots \texttt{y}}}{\texttt{f}}}$ is
translated to a C function.
The function needs to refernce $\texttt{y}$.
Conceptually, $\texttt{f}$ is a closure which contains $\texttt{y}$.
But \codegen{} translates fixpoints without closures:
\codegen{} adds extra arguments, $\texttt{y}$ to the function.
We call such argument as outer-variable.

We define $\FIXFUNCS$, $\FIXFV$, $\OVARS'$, and $\OVARS$ for each definition \kwDefinition~$c := u$.

$\OVARS(f)$ is the outer-variables of a fix-bounded function $f$ in the definition.
...
\[
  \FIXFV(x) =
  \begin{cases}
    \FV(F) & \text{$u$ contains $F$ where $F = \ofix{\repi{(f_l:=t_l)}{1\leq l < i} (x:=t_i) \repi{(f_l:=t_l)}{i < l \leq h}}{f_j}$}  \\
    \varnothing & \text{otherwise}
  \end{cases}
\]

$\OVARS'(x)$ is a set which satisfy the following condition.
$\OVARS'(f)$ is similar to $\FIXFV(f)$ which is the set of free variables of the fixpoint which bounds $f$.
But if it contains a function bounded by an outer fixpoint, the free variable of the outer fixpoint are also contained.

\[
  \OVARS'(x) \supseteq \FIXFV(x) \cup \bigcup_{y \in \FIXFV(x)} \OVARS'(y)
\]

\codegen{} chooses the minimal set for $\OVARS'(f)$ if a dedicated internal C function is generated for $f$.
...
This means $\texttt{f}\:\texttt{y'}$ is translated to $\texttt{c}\:\texttt{x}\:\texttt{y'}$.
In this case, the arguments of the external C function for $c$ must corresponds to the type of $c$.
Thus \codegen{} chooses the maximal set (all bound variables) for $\OVARS'(f)$.
...
$\OVARS(x)$ is defined as follows.
It is $\OVARS'(x)$ except fix-bounded functions.

\[
  \OVARS(x) = \OVARS'(x) - \FIXFUNCS
\]

When $\OVARS(x)$ is used in a context which the order matters,
we consider it is a list of variables from declared ouside to inside.
(used in \secref{sec:AK})

When $\OVARS(x)$ is used in a context which require types,
...
$f_i$ invocation in C needs extra arguments, $\OVARS(f_i) = y_{i1}\mathord{:}U_{i1} \ldots y_{io_i}\mathord{:}U_{io_i}$, addition to the actual arguments in \gallina{} application because the free variables of the fixpoint should also be passed.
