...
\DeclareMathOperator{\FLARGS}{FLARGS}
...
Definition c x y :=
  ...
  | S n' =>
    (fix g m :=
    match m with
    | O => y + f n'         (* g references y and f *)
    | S m' => S (g m')
    end) n
  end.
\end{lstlisting}

\codegen{} translate a Gallina application to C function call.
In this scenario, internal functions \lstinline[style=Cstyle]!g! and \lstinline[style=Cstyle]!f! has corresponding C functions.
\lstinline!g m'! is translated to \lstinline[style=Cstyle]!g(m')! where \lstinline[style=Cstyle]!g! is a C function.
But it does not work because \lstinline!g! needs \lstinline!y!.
Also, \lstinline!g! needs \lstinline!f!.
Although there is a C function \lstinline!f!, \lstinline!f! needs \lstinline!x!.
Thus, \codegen{} need to add extra arguments as \lstinline[style=Cstyle]!g(x, y, m')! which is similar to lambda-lifting.
We call such argument as fix-lifting-arguments.

We define $\FIXFUNCS$, $\FIXFV$, $\FLARGS'$, and $\FLARGS$ for each definition \kwDefinition~$c := u$.

$\FLARGS(f)$ is the fix-lifting-arguments of a fix-bounded function $f$ in the definition.
...
$\FLARGS'(x)$ is a set which satisfy the following condition.
$\FLARGS'(f)$ is similar to $\FIXFV(f)$ which is the set of free variables of the fixpoint which bounds $f$.
But if it contains a function bounded by an outer fixpoint, the free variable of the outer fixpoint are also contained.

\[
  \FLARGS'(x) \supseteq \FIXFV(x) \cup \bigcup_{y \in \FIXFV(x)} \FLARGS'(y)
\]

\codegen{} chooses the minimal set for $\FLARGS'(f)$ if a dedicated internal C function is generated for $f$.
...
Thus \codegen{} chooses the maximal set (all bound variables) for $\FLARGS'(f)$.
...
$\FLARGS(x)$ is defined as follows.
It is $\FLARGS'(x)$ except fix-bounded functions.

\[
  \FLARGS(x) = \FLARGS'(x) - \FIXFUNCS
\]

When $\FLARGS(x)$ is used in a context which the order matters,
we consider it is a list of variables from declared ouside to inside.
(used in \secref{sec:AK})

When $\FLARGS(x)$ is used in a context which require types,
...
$f_i$ invocation in C needs extra arguments, $\FLARGS(f_i) = y_{i1}\mathord{:}U_{i1} \ldots y_{il_i}\mathord{:}U_{il_i}$, addition to the actual arguments in \gallina{} application because the free variables of the fixpoint should also be passed.
...
\[ \auxstructdef{c}{i} = \dq{\kwstruct\:\mathtt{arg\_}f_i\:\ttbrace{\:\repi{U_{ij}\:\mathtt{fl}j\ttsemi}{j=1\ldots l_i}\:\repi{T_{ij}\:\mathtt{arg}j\ttsemi}{j=1\ldots m_i}\:}\ttsemi} \]
...
\begin{align*}
  ...
  & \repi{y_{ij}\:\tteq\:\ttparen{\ttparen{\kwstruct\:\mathtt{arg\_}f_i\:\ttstar}\mathtt{arg}}\texttt{->}\mathtt{fl}j\ttsemi}{j=1\ldots l_i} \\
