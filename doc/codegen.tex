...
$\KV(K)$ means the bound variables usable in the hole of the context $K$.

\begin{align*}
  \KV([]) &= \varnothing \\
  \KV(\lamT{x}{T}{K}) &= \{x\} \cup \KV(K) \\
  \KV(K\:t) &= \KV(K) \\
  \KV(t\:K) &= \KV(K) \\
  \KV(\letin{x}{K:T}{t}) &= KV(K) \\
  \KV(\letin{x}{t:T}{K}) &= \{x\} \cup \KV(K) \\
  \KV(\omatch{K}{\rep{C\:\rep{x{:}T}\Rightarrow u}}) &= \KV(K) \\
  \KV(\omatch{t}{\repi{(C_l\:\rep{x_l{:}T_l}\Rightarrow u_l)}{1\leq l < i} (C_i\:\rep{x_i{:}T_i}\Rightarrow K) \repi{(C_l\:\rep{x_l{:}T_l}\Rightarrow u_l)}{i < l \leq h}} &= \{\rep{x_i}\} \cup \KV(K) \\
  \KV(\ofix{\repi{(f_l/k_l{:}T_l:=t_l)}{1\leq l < i} (f_i/k_i{:}T_i:=K) \repi{(f_l/k_l{:}T_l:=t_l)}{i < l \leq h}}{f_j}) &= \{\rep{f}\} \cup \KV(K)
\end{align*}
