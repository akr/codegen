...
\begin{gather*}
  \text{beta:}~
    E[\Gamma] \vdash ((\lam{x}{t})~u)~\triangleright~\subst{t}{x}{u} \\
  \text{delta-local:}~
    \dfrac{(x:=t) \in \Gamma}{E[\Gamma] \vdash x~\triangleright~t} \\
  \text{delta-global:}~
    ...
    \dfrac
    {
      ...
      \begin{aligned}
        E[\Gamma] &\vdash (\fix{f_i/k_i}{t_i}{i=1\dots h}{f_j})~x_1\ldots x_n ~\triangleright~ \\
                  &\quad \letinB{f'_1}{\fix{f_i/k_i}{t_i}{i=1\dots h}{f_1}} ~\dots \\
                  &\quad \letinB{f'_h}{\fix{f_i/k_i}{t_i}{i=1\dots h}{f_h}} \\
...
\begin{align*}
  ...
  &\A{K}{x}{x_1\dots x_n} =
    ...
    && \begin{aligned}[t] & n > 0 \wedge \text{$x$ is bounded by a fixpoint} \mathop{\wedge} \\ & x \in \tr \end{aligned} \\
  &\A{K}{x}{x_1\dots x_n} = K(\dq{x\ttparen{y_1,\dots,y_o,x_1,\dots, x_n}})
    && \begin{aligned}[t] & n > 0 \wedge \text{$x$ is bounded by a fixpoint} \mathop{\wedge} \\ & x \not\in \tr \end{aligned} \\
...
\[
  ...
  \begin{cases}
    ...
    \B{K}{t}{}                            & \begin{aligned}[t]
                                            & \text{otherwise} \\
                                            & \text{where}~t:T \quad K(e) = \dq{\texttt{*($T$*)ret = $e$; \kwCreturn;}}
                                            \end{aligned}
  \end{cases}
\]
\[
