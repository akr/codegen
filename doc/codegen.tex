...
\begin{itemize}
  ...
  \item We may write the range part, $i=1\ldots n$, as $1\leq i \leq n$:
    $x_1+y_1 < \ldots < x_n+y_n$ means $\repopi{x_i+y_i}{<}{1\leq i \leq n}$.
...
\begin{gather*}
  \text{verify-matchapp}:
    \dfrac{
      \rep{\underline{E[\Gamma]} \vdash p : \forall \mathit{cargs}, t\:\underline{\mathit{args}_1} = u\:\underline{\mathit{args}_2}}
    }{
      E[\Gamma] \vdash
        \begin{aligned}[t]
          ...
          & \quad
              \begin{aligned}
                & \kwmatch\:x'\:\kwwith\:\rep{C\:\mathit{cargs} \Rightarrow t}\:\kwend\:\mathit{args}_1 = \\
                & \kwmatch\:x'\:\kwwith\:\rep{C\:\mathit{cargs} \Rightarrow u}\:\kwend\:\mathit{args}_2
              \end{aligned} \\
  ...
  \text{verify-fix}:
    \dfrac{
      \begin{aligned}
        ...
        & \forall h \in \{\rep{g}\}, (\FV(t_h) \cup \FV(u_h)) \cap (\{\rep{f}\} \backslash \{\rep{g}\}) = \varnothing \quad \mbox{($\{\rep{f}\} \backslash \{\rep{g}\}$ are unreachable from $f_s$)} \\
        % & h_1 = |\rep{f}| \\
        % & h_2 = |\rep{g}| \\
        & \forall h \in \{\rep{g}\}, E[\Gamma]; \repi{\mathit{IH}_e : F_e \simeq G_e}{e\in\{\rep{g}\}} \vdash p_{h} : \substmi{t_{h}}{F_e}{e}{e\in\{\rep{f}\}} \simeq \substmi{u_{h}}{G_e}{e}{e\in\{\rep{g}\}}
      \end{aligned}
    }{
      \begin{aligned}
        & (\kwfix\:\repi{\mathit{IH}_e / k_e : (F_e \simeq G_e) := q}{e\in\{\rep{g}\}}\:\kwfor\:\mathit{IH}_{f_s})\:\mathit{aargs} \\
        & : F_{f_s}\:\mathit{aargs} = G_{f_s}\:\mathit{aargs}
      \end{aligned}
    } \\
    \quad \mbox{where} \quad
      \begin{aligned}[t]
        & F_{h} := \kwfix\:\rep{f/k_f:=t_f}\:\kwfor\:{h} \\
        & G_{h} := \kwfix\:\rep{g/k_g:=u_g}\:\kwfor\:{h} \\
        & \mbox{$\{\rep{f}\}$ is the fix-bounded function names in the LHS fixpoint, $F_{f_s}$} \\
        & \mbox{$\{\rep{g}\}$ is the fix-bounded function names in the RHS fixpoint, $G_{f_s}$} \\
        & \mbox{$k_h$ is the index of the decreasing argument of $h$} \\
        & \mbox{$t_h$ is the function term of the function name $h$ in the LHS fixpoint} \\
        & \mbox{$u_h$ is the function term of the function name $h$ in the RHS fixpoint} \\
        & \mbox{$f_s$ specifies the start function of the LHS and RHS fixpoint} \\
        & \mbox{$\mathit{params}_h$ is the inductive type parameters of the decreasing argument of $h$} \\
        & q_h :=
          \begin{aligned}[t]
            & \lambda x_1 \ldots x_{\NA_{h}} . \\
            & \kwlet\:H := p_{h}\:\kwin \\
            & \kwmatch\:x_{k_{h}}\:\kwas\:y\:\kwreturn \\
            & \quad F_{h}\:x_1 \ldots x_{k_{h}-1}\:y\:x_{k_{h}+1} \ldots x_{\NA_{h}} = G_{h}\:x_1 \ldots x_{k_{h}-1}\:y\:x_{k_{h}+1} \ldots x_{\NA_{h}} \\
            & \kwwith \\
            & \rep{|\:C\:\mathit{cargs} \Rightarrow \underline{H\:x_1 \ldots x_{k_{h}-1}}\:(C\:\mathit{params}\:\mathit{cargs})\:\underline{x_{k_{h}+1} \ldots x_{\NA_{h}}}} \\
            & \kwend
          \end{aligned}
      \end{aligned} \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        This verify-fix rule builds a proof term ($\kwfix\ldots\kwfor\:\mathit{IH}_{f_s}$) for extensional equality of fixpoints ($F_{f_s} \simeq G_{f_s}$) from extensional equality proofs of iota-fix reduced forms ($p_h$).
        The \kwmatch-expression in $q_h$ makes the equality iota-fix reducible by destructing the decreasing argument.
      \end{minipage}
    \breakrule
\end{gather*}
...
\begin{gather*}
  \text{verify-apparg}:
    \dfrac{
      ...
      \kwrew\: p\: \kwin\: q : t_1\: \mathit{args}_0\: x_1\: \mathit{args}_1 = t_2\: \mathit{args}_0\: x_2\: \mathit{args}_2
    } \\
    \quad \mbox{where} \quad \kwrew\: p\: \kwin\: q := \mbox{eq\_ind}\: x_1\: (\lam{x}{t_1\: \mathit{args}_0\: x\: \mathit{args}_1})\: q\: x_2\: p \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        When x1 and x2 are functions, p is an equality of functions.  It needs functional extensionality.
      \end{minipage}
\end{gather*}
...
\begin{gather*}
  \text{funext-ind}:
    \dfrac{
      \begin{aligned}
        & \mbox{$t_1$ and $t_2$ are not a function} \\
        & E[\Gamma] \vdash p : t_1 \simeq t_2 \\
      \end{aligned}
    }{
      p : t_1 = t_2
...
\begin{gather*}
  \text{funext-fun}:
    \dfrac{
      \begin{aligned}
        & \mbox{$t_1$ and $t_2$ are a function} \\
...
The axiom, functional\_extensionality, is required when different functions are bounded with \kwlet{} and they are used as a function argument.
The verify-letin-generic rule introduces $x_1 \simeq x_2$ which is not usable for eq\_ind which require $x_1 = x_2$.
