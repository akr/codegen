...
\begin{gather*}
  \text{verify-fix}:
    \dfrac{
      \begin{aligned}
        & \forall i \in \{1\ldots h'\}, f_{\sigma(i)} = f'_i \\
        & \forall i \in \{1\ldots h'\}, k_{\sigma(i)} = k'_i \\
    ...
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
        This verify-fix rule builds a proof term ($\kwfix\ldots\kwfor\:\mathit{IH}_{s'}$) for extensional equality of fixpoints ($F_{s} \simeq F'_{s'}$) from extensional equality proofs of iota-fix reduced forms ($p_i$).
