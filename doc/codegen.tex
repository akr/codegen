...
\begin{itemize}
  ...
  \item Nested overline and underline can be combined. \\
    ...
    We cannot define how to repeat $\rep{(\underline{b})}$ because no variable without underline.
...
\begin{align*}
  t =&\ x & \text{variable} \\
    ...
    |&\ \omatch{t}{\rep{C\:\rep{x{:}T}\Rightarrow u}} & \text{conditional} \\
    |&\ \ofix{\rep{f/k{:}T:=t}}{f_j} & \text{fixpoint}
\end{align*}
{\small Note:
\begin{itemize}
  ...
  \text{etaex-fix:}~
    \dfrac{
      \begin{aligned}
        ...
        & K = \ofix{\repi{(f_l{:}T_l:=t_l)}{1\leq l < i} (f_i{:}T_i:=[]) \repi{(f_l{:}T_l:=t_l)}{i < l \leq h}}{f_j}
      \end{aligned}
    }{
...
\begin{gather*}
  ...
  \text{delta-fun:}~
    \dfrac
    {
      0 \leq |x| \quad
      0 < |y| \quad
      (f := t\:\rep{x}) \in \Gamma \quad
      \text{$t$ is one of $x, c, C, (\lam{x}{u}), (\ofix{\rep{f/k:=t}}{f_j})$}
    }{
...
\begin{align*}
  ...
  \RNT{\omatch{y}{\rep{C\:\rep{x}\Rightarrow t}}} &= \bigl(\tbigcup_i \TR{t_i}, \tbigcup_i \N{t_i}, \tbigcup_i \T{t_i} \bigr) \\
...
\begin{itemize}
  ...
  \item $y$ of $\omatch{y}{\rep{C\:\rep{x}\Rightarrow t}}$ is not counted because $y$ is not a function and does not affect the final result.
\end{itemize}}
