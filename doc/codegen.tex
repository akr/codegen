      ...
      \begin{aligned}[t]
        ...
        & q_j :=
          \begin{aligned}[t]
            ...
            & \kwend
          \end{aligned} \\
        & \mbox{$\mathit{cargs}$ is variables for the constructors members}
      \end{aligned} \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
...
\begin{gather*}
  \text{verify-apparg}:
    ...
    \quad \mbox{where} \quad \kwrew\: p\: \kwin\: q := \mathsf{eq\_ind}\: x\: (\lam{y}{t\: \mathit{args}_0\: x\: \mathit{args} = t'\: \mathit{args}_0\: y\: \mathit{args}'})\: q\: x'\: p \\
    \quad \mbox{Note:}~
      \begin{minipage}[t]{0.8\columnwidth}
...
The axiom, $\mathsf{functional\_extensionality}$, is required when different functions are bounded with \kwlet{} and they are used as an argument for an application.
