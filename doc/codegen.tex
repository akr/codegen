...
\begin{itemize}
  \item We use overline to represent repetition:
    $\overline{x_i+y_i}^{1\leq i \leq n}$ means $x_1+y_1, \ldots, x_n+y_n$.
  \item We assume appropriate separator for the context (``,'' in the above example) are inserted.
    (The separator may inserted at beginning or end if appropriate.)
  \item We omit indexes when the range of index are known:
    $\overline{x+y}$ means $\overline{x_i+y_i}^{1\leq i \leq n}$ if $x$ and $y$ are $n$-element sequences.
    The index is added to the all meta-variables under the overline
  \item We use nested overline to represent two- or more-dimensional indexes.
    $\omatch{t}{\overline{C\:\overline{x}\Rightarrow u}}$ means
    $\omatch{t}{ C_1\: x_{11}\ldots x_{1m_1} \Rightarrow u_1
                \:|\: C_2\: x_{21}\ldots x_{2m_2} \Rightarrow u_2
                \:|\: \ldots
                \:|\: C_n\: x_{n1}\ldots x_{nm_n} \Rightarrow u_n}$.
  \item We use underline to suppress adding indexes.
    $\overline{\letinB{f'}{\ofix{\underline{(\overline{f/k:=t})}}{f}}}$ means \\
    $\letinB{f'_1}{\ofix{\overline{(f/k:=t)}}{f_1}} ~\ldots~
     \letinB{f'_h}{\ofix{\overline{(f/k:=t)}}{f_h}}$.
