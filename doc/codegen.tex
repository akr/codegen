...
We define a judgement $E[\Gamma] \vdash t~[q]~\{x_1 \mapsto n_1, \dotsc, x_m \mapsto n_m\}$ for linearity check.
It means that a free linear variable $x_i$ in $t$ is referenced $n_i$ times when $t$ is evaluated with $q$ arguments.

We require $E[\Gamma] \vdash t'~[\NA_t]~\varnothing$ when \kwDefinition~$c := t$ is transformed to $t'$ by the convertible transformations until argument completion.
