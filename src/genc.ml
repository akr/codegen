...
(*
  ...
  The result is stored in fixinfo as side effect.
...
and collect_fix_usage1 ~(inlinable_fixterms : bool Id.Map.t)
  ...
  | Rel i ->
      (if 0 < numargs then
        ...
        | Some inlinable ->
            let info = Hashtbl.find fixinfo id in
            let info2 =
            ...
            in
            Hashtbl.replace fixinfo id info2)
...
let rec detect_top_fixterms_rec
    ~(fixinfo : fixinfo_t)
    (env : Environ.env) (sigma : Evd.evar_map)
    (term : EConstr.t) (numargs : int) (tail_position : bool) : top_fixterm_t Seq.t =
  detect_top_fixterms_rec1 env sigma ~fixinfo term numargs tail_position
and detect_top_fixterms_rec1
  ...
  | Proj _ -> Seq.empty
  | App (f, args) ->
      detect_top_fixterms_rec ~fixinfo env sigma f
        (Array.length args + numargs) tail_position
  | LetIn (x,e,t,b) ->
      let decl = Context.Rel.Declaration.LocalDef (x, e, t) in
      let env2 = EConstr.push_rel decl env in
      Seq.append
        (detect_top_fixterms_rec ~fixinfo env sigma e 0 false)
        (detect_top_fixterms_rec ~fixinfo env2 sigma b numargs tail_position)
  | Case (ci, p, iv, item, branches) ->
      seq_flat_map2
        (fun cstr_nargs br ->
          detect_top_fixterms_rec ~fixinfo env sigma br (cstr_nargs + numargs) tail_position)
  ...
  | Lambda (x,t,b) ->
      ...
      if numargs = 0 then
        ...
        detect_top_fixterms_rec ~fixinfo env2 sigma b (numargs-1) tail_position
  | Fix ((ia, i), ((nary, tary, fary) as prec)) ->
      let env2 = EConstr.push_rec_types prec env in
      if tail_position then
        seq_flat_map2
          (fun t f ->
            ...
            detect_top_fixterms_rec ~fixinfo env2 sigma f numargs2 tail_position)
        ...
        if usage.fixfunc_inlinable then
          ...
          Seq.cons
            ...
            (seq_flat_map2
              (fun t f ->
                let numargs2 = numargs_of_type env sigma t in
                detect_top_fixterms_rec ~fixinfo env2 sigma f numargs2 true)
              (Array.to_seq tary) (Array.to_seq fary))
...
let detect_top_fixterms
    ...
    top_fixterm_t list =
  let numargs = numargs_of_exp env sigma term in
  List.of_seq (detect_top_fixterms_rec ~fixinfo env sigma term numargs true)
