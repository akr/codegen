...
let pr_codegen_instance (env : Environ.env) (sigma : Evd.evar_map) (sp_cfg : specialization_config) (sp_inst : specialization_instance) : Pp.t =
  ...
  let static_args = List.filter_map (fun arg_opt -> arg_opt) sp_inst.sp_static_arguments in
  match sp_inst.sp_interface with
  | None ->
      Pp.str "CodeGen NoFunc" +++ pr_constant_or_constructor env sp_cfg.sp_func ++ Pp.str "."
  | Some sp_interface ->
      ...
      (if sp_interface.sp_presimp_constr = func then
        Pp.str "_"
      else
        Printer.pr_constr_env env sigma sp_interface.sp_presimp_constr) +++
      ...
      | Some sp_interface ->
          ...
          | Some { sp_simplified_status=(SpDefined (ctnt, referred_cfuncs)) } ->
              msg_info_hov (Pp.str "Dependency" +++
                Pp.str sp_interface.sp_cfunc_name +++
                Pp.str "=>" +++
...
let check_cfunc_name_conflict env sigma icommand presimp cfunc_name =
  ...
  | (CodeGenFunc), Some (CodeGenCfuncPrimitive _) ->
      user_err
        (Pp.str "[codegen] C function name already used:" +++
        Pp.str cfunc_name +++
  ...
  | (CodeGenPrimitive | CodeGenConstant), Some (CodeGenCfuncGenerate (sp_cfg, sp_inst, sp_interface, sp_gen)) ->
      user_err
        (Pp.str "[codegen] C function name already used:" +++
        Pp.str cfunc_name +++
        Pp.str "for" +++
...
let make_presimp_for_instance
  ...
  in
  (env, sigma, sp_cfg, static_args, presimp, presimp_constr, s_id, cfunc_name)

let register_sp_instance ?(cfunc : string option)
    (env : Environ.env) (sigma : Evd.evar_map)
    (func : Constr.t) (presimp : Constr.t)  (sp_cfg : specialization_config) (sp_inst : specialization_instance) : unit =
  let inst_map = ConstrMap.add presimp sp_inst sp_cfg.sp_instance_map in
  let sp_cfg = { sp_cfg with sp_instance_map = inst_map } in
  let sp_interface = sp_inst.sp_interface in
  let sp_gen =
    match sp_inst.sp_interface with
  ...
  in
  add_specialize_config func sp_cfg;
  add_gallina_instance_specialization presimp sp_cfg sp_inst;
  ...
  | (Some sp_interface), (Some sp_gen) ->
      add_cfunc_instance sp_interface.sp_cfunc_name (CodeGenCfuncGenerate (sp_cfg, sp_inst, sp_interface, sp_gen))
  | (Some sp_interface), None ->
      let cfunc_name = sp_interface.sp_cfunc_name in
      (match CString.Map.find_opt cfunc_name (get_cfunc_instance_map ()) with
      | None -> add_cfunc_instance cfunc_name (CodeGenCfuncPrimitive [(sp_cfg, sp_inst)]);
      | Some (CodeGenCfuncPrimitive l) -> add_cfunc_instance cfunc_name (CodeGenCfuncPrimitive ((sp_cfg, sp_inst)::l))
...
let codegen_instance_command_nofunc
  ...
  let sp_inst = make_sp_inst sigma presimp static_args None icommand in
...
let aenv_push_assum (aenv : aenv_t) (x : Names.Name.t EConstr.binder_annot) (t : EConstr.types) : aenv_t = {
    aenv_env = env_push_assum aenv.aenv_env x t;
    aenv_fix_bounded = false :: aenv.aenv_fix_bounded;
  }

let aenv_push_assums (aenv : aenv_t) (assums : (Names.Name.t EConstr.binder_annot * EConstr.types) list) : aenv_t = {
    aenv_env = env_push_assums aenv.aenv_env assums;
    aenv_fix_bounded = CList.addn (List.length assums) false aenv.aenv_fix_bounded;
  }

let aenv_push_def (aenv : aenv_t) (x : Names.Name.t EConstr.binder_annot) (e : EConstr.t) (t : EConstr.types) : aenv_t = {
    aenv_env = env_push_def aenv.aenv_env x e t;
    aenv_fix_bounded = false :: aenv.aenv_fix_bounded;
  }

let aenv_push_defs (aenv : aenv_t) (defs : (Names.Name.t EConstr.binder_annot * EConstr.t * EConstr.types) list) : aenv_t = {
    aenv_env = env_push_defs aenv.aenv_env defs;
    aenv_fix_bounded = CList.addn (List.length defs) false aenv.aenv_fix_bounded;
  }

let aenv_push_fix (aenv : aenv_t) (prec : EConstr.rec_declaration) : aenv_t =
...
and normalizeV1 (aenv : aenv_t) (sigma : Evd.evar_map) (term : EConstr.t) : EConstr.t =
  let wrap_lets hoisted_exprs lifted_term =
    ...
    let hoisted_names = List.map (fun ty -> EConstr.nameR (Id.of_string (Namegen.hdchar aenv.aenv_env sigma ty))) hoisted_types in
...
let test_bounded_fix (env : Environ.env) (sigma : Evd.evar_map) (n : int)
    ...
    (prec : Name.t EConstr.binder_annot array * EConstr.types array * EConstr.t array) =
...
let replace_app ~(cfunc : string) (env : Environ.env) (sigma : Evd.evar_map) (func : Constr.t) (args : EConstr.t array) : Environ.env * EConstr.t * string =
  ...
  (env, newexp, sp_interface.sp_cfunc_name)
...
let rec formal_argument_names (env : Environ.env) (sigma : Evd.evar_map) (term : EConstr.t) : Name.t EConstr.binder_annot list =
  match EConstr.kind sigma term with
  | Lambda (x,t,e) ->
      let env2 = env_push_assum env x t in
...
let codegen_simplify (cfunc : string) : Environ.env * Constant.t * StringSet.t =
  ...
  let env = Global.env () in
  let sigma = Evd.from_env env in
  let s_id = match sp_gen with
    | { sp_simplified_status=(SpExpectedId s_id) } -> s_id
    | { sp_simplified_status=(SpDefined _) } -> user_err (Pp.str "[codegen] already simplified:" +++ Pp.str cfunc)
  in
  ...
  let sp_interface2 = {
    ...
    sp_cfunc_name = sp_interface.sp_cfunc_name;
    sp_gen = Some sp_gen2;
  }
  in
  let sp_inst2 = {
    ...
    sp_interface = Some sp_interface2;
...
let rec recursive_simplify (visited : StringSet.t ref) (rev_postorder : string list ref) (cfunc : string) : unit =
  if StringSet.mem cfunc !visited then
    ...
    | Some (CodeGenCfuncGenerate (sp_cfg, sp_inst, sp_interface, sp_gen)) ->
        (match sp_gen with
        | { sp_simplified_status=(SpExpectedId _) } ->
